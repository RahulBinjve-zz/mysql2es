...
class MySQLToElasticSearch:
    ...
    """
    def __init__(self, conf_file="mysql2es.conf"):
        """ Setting up required variables.
        """

        self.__config = self.parse_config(conf_file)
    ...
    def get_data(self, structure, size=10):
        result = []
    ...
    def push_to_elasticsearch(self, data, doc_type):
        ...
        try:
            ...
            for key in data.keys():
                ...
                data[key] = unicode(str(data[key]), errors='replace')
            data = json.dumps(data)
...
def main(conf_file):
    try:
        my_dump = MySQLToElasticSearch(conf_file=conf_file)
        total = 0
        for table_struct in my_dump.structure:
            print "\n\n[+] Fetching data for %s table." % \
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Dumps MySQL database to ElasticSearch.')
    parser.add_argument('-c', '--config', 
                        help='Config file to use. \
                        It should be similar to the one provided. \
                        Defaults to "mysql2es.conf"',
                        default="mysql2es.conf")
